...
drop function public.create_studio_xp_correction(uuid, integer, text);
...
declare
  ...
  existing_event public.xp_events%rowtype;
...
begin
  ...
  where organization_id = target_organization_id
    and idempotency_key = 'correction:' || correction_id::text;

  if existing_event.id is not null then
    return jsonb_build_object(
      'event_id', existing_event.id,
      'previous_total', (existing_event.metadata ->> 'previous_total')::bigint,
      'new_total', (existing_event.metadata ->> 'new_total')::bigint,
      'previous_level', (existing_event.metadata ->> 'previous_level')::integer,
      'new_level', (existing_event.metadata ->> 'new_level')::integer
    );
  end if;
...
revoke all on function public.create_studio_xp_correction(
  uuid, uuid, integer, text
) from public, anon;
grant execute on function public.create_studio_xp_correction(
  uuid, uuid, integer, text
) to authenticated;
